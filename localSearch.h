    ...
    int DTS_v()
{
	...
	int i,e;
    int v1,v2,v;
	int best_e = rand() % e_num;
	
	for (i=1; i<cand_count; ++i)
	{
		e = rand() % e_num;
	
		if( edges[e].get_weight() > edges[best_e].get_weight()) best_e = e;
...
#ifdef DTS_Cvoid_mode
                v = DTS_v();

#else

				v = rand() % v_num + 1;    //try   C=NULL SET, DTS
#endif
...
public:
	...
	void show_results()
	{
		...
		if (check_solution())
		{
						cout << "o " << LL(best_clique_weight) << endl;
			...
			//cout << best_solve_step << endl;
			//cout << step << endl;
			//cout << setprecision(3) << step / elap_time << endl;
