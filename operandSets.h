...
public:	
	...
	int best_element(int* swap_partner, int* confChange, long double* score, HugeInt* time_stamp)
	{
		...
		for(; i != end(); i++)
		{
			int e = array[i];
			if(!confChange[e]) continue;
			if(long_double_greater(score[e] + score[swap_partner[best_e]], score[best_e] + score[swap_partner[e]]) || (long_double_equals(score[e] + score[swap_partner[best_e]], score[best_e] + score[swap_partner[e]]) && time_stamp[e] < time_stamp[best_e]))
				best_e = e;
		}
		return best_e;
	}
